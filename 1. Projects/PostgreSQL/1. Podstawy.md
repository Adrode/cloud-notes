...
## Jak działa IS?
#IS 
- **`SELECT ... WHERE country IS NULL;`** - `IS` służy do sprawdzenia czy coś jest `NULL`, ponieważ `=` w takich sytuacjach zwraca `UNKNOWN`, czyli false, co jest zawodne w takich porównaniach
## Jak działa BETWEEN?
#BETWEEN
...
## Jak działa alias (AS)?
#AS
- **`SELECT *, price AS prajs FROM car;`** - zmienia nazwę wskazanej kolumny
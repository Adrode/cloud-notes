...
- **Recursion** - wywołanie funkcji wewnątrz jej samej, np. ciąg Fibonacciego
	- ```python
	  fibo_elements = [0, 1] # początkowe liczby ciągu
	  for i in range(10): # 10 powtórzeń
		  fibo_elements.append(fibo_elements[-1] + fibo_elements[-2])
	  print(fibo_elements) # pierwsze 12 elementów ciągu (10 + 2 początkowe)
	  ```
		- tutaj jest to pętla, ale metoda append na `fibo_elements` wywołuje odniesienie do elementów `fibo_appends`, czyli jednocześnie operując na tej liście odwołuję się do jej aktualnych elementów
- **Call Stack** -




[Data Structures & Algorithms](https://www.youtube.com/watch?v=TGT79h7e7tE&list=PLKYEe2WisBTFEr6laH5bR2J19j7sl5O8R&index=7)